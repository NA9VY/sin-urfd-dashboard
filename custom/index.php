...
<!--                <li class="navbar-info">NA9VY Dashboard v<?php echo $PageOptions['DashboardVersion']; ?></li> -->
                <li class="navbar-info"><?php echo $PageOptions['UptimePhrase']; ?>: <?php echo FormatSeconds($Reflector->GetServiceUptime()); ?></li>
...
<!-- Lightning Status with Auto-Expire + Timer -->
<?php
$lightningFile = '/home/pi/lightning_status.txt';
$expireMinutes = 30; // How long to show the alert

if (file_exists($lightningFile)) {
    $data = json_decode(file_get_contents($lightningFile), true);
    
    if ($data && isset($data['level']) && isset($data['timestamp'])) {
        
        // Get last strike time (supports both Unix timestamp and string)
        $lastTime = is_numeric($data['timestamp']) 
        ...
        if ($lastTime > 0) {
            ...
            if ($minutesAgo <= $expireMinutes && $minutesAgo >= 0) {
                $level = $data['level'];
                $text = $data['text'] ?? 'Lightning Detected';
                ...
                // Choose color style
                $style = match($level) {
                    'red'    => 'background:#8B0000; color:#ffdddd;',
                    'yellow' => 'background:#B8860B; color:#ffffcc;',
                    default  => 'background:#006400; color:#ccffcc;'
                };

                echo '<span style="' . $style . ' padding:5px 12px; border-radius:6px; font-weight:bold; margin-left:8px;">';
                echo '⚡ ' . htmlspecialchars($text) . ' <small>(' . $timeAgo . ')</small>';
                echo '</span>';
            }
        }
    }
...
// Helper function: Format minutes into "X minutes ago", "1 hour ago", etc.
    ...
    <div class="row">
        <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
                ...
                <li<?php echo ($_GET['show'] == "repeaters") ? ' class="active"' : ''; ?>><a href="./index.php?show=repeaters">Connected<br>Devices (<?php echo $Reflector->NodeCount(); ?>)</a></li>
                <li<?php echo ($_GET['show'] == "peers") ? ' class="active"' : ''; ?>><a href="./index.php?show=peers">Interlinks (<?php echo $Reflector->PeerCount(); ?>)</a></li>
                <li<?php echo ($_GET['show'] == "modules") ? ' class="active"' : ''; ?>><a href="./index.php?show=modules">Modules List</a></li>
                <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-----------------------<br>
                 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;RPi & Network Stats<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;for SIN</p>

    <!-- Lightning Map -->
    <li<?php echo ($_GET['show'] == "lightning-map") ? ' class="active"' : ''; ?>>
        <a href="./index.php?show=lightning-map">⚡ Lightning Map</a>
    </li>

		<li<?php echo ($_GET['show'] == "pidashboard") ? ' class="active"' : ''; ?>><a href="https://<?php echo $PageOptions['PiDashboard']; ?>/" target="_blank">Pi Dashboard</a></li>
	        <li<?php echo ($_GET['show'] == "ysf-auto-reconnect") ? ' class="active"' : ''; ?>><a href="./index.php?show=ysf-auto-reconnect">YSF Auto-Reconnect</a></li>
                <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-----------------------<br>
                 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;BrandMeister Links<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;for SIN</p>
		<li<?php echo ($_GET['show'] == "lastheardsin") ? ' class="active"' : ''; ?>><a href="https://brandmeister.network/?page=lh&DestinationID=<?php echo $PageOptions['BrandmeisterTG']; ?>" target="_blank">Last Heard<br>on BM SIN</a></li>
		<li<?php echo ($_GET['show'] == "devicessin") ? ' class="active"' : ''; ?>><a href="https://brandmeister.network/?page=talkgroup&id=<?php echo $PageOptions['BrandmeisterTG']; ?>" target="_blank">Devices connected<br>to BM SIN</a></li>

                <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-----------------------<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Links to other stuff</p>
                <li<?php echo ($_GET['show'] == "reflectors") ? ' class="active"' : ''; ?>><a href="https://dvref.com/" target=blan>Digital Voice Reflectors</a></li>
                <li<?php echo ($_GET['show'] == "livequadnet") ? ' class="active"' : ''; ?>><a href="./index.php?show=livequadnet">D-Star Activity Live</a></li>
            </ul>
        </div>
         ...
         <div class="error">
            ...
            switch ($_GET['show']) {
                ...
                case 'reflectors' :
                    ...
                    break;
                case 'ysf-auto-reconnect' :
                    require_once("./pgs/ysf-auto-reconnect.php");
                    break;
                case 'lightning-map' :
                    require_once("./lightning_map.php");
                    break;
                default :
                    require_once("./pgs/users.php");
            }
